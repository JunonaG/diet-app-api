...
const knex = (0, import_knex.knex)(config)
...
async function userRoutes(app2) {
  ...
  app2.get('/:id', async (request, reply) => {
    try {
      const getUserInfoByIdSchema = import_zod2.z.object({
  ...
  app2.get('/details/:email', async (request, reply) => {
    try {
      const getUserInfoByEmailSchema = import_zod2.z.object({
...
const import_crypto2 = require('crypto')

// src/middleware/check-if-session-exists.ts
const checkIfSessionExists = async (request, reply) => {
...
async function mealRoutes(app2) {
  ...
  app2.get('/streak/:userId', async (request, reply) => {
    try {
      const streakGenerator = function (array2) {
        let streak = 0
        let currentStreak = 0
